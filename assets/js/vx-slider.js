...
/*!
* 	VX Slider
* 	https://github.com/JustinGlueck/vx-slider
* 	Copyright 2015 Justin Glueck
...
;( function($){
	...
	$.fn.vxSlider = function( options ) {
		...
		var bindTouchSwipe = function() {
			var startY = null;
			$(window).on( "touchstart.vxSlider", function(ev) {
			...
			$(window).on( "touchmove.vxSlider", function(ev) {
    			var e = ev.originalEvent;
				if( startY !== null ) {
					var touches = e.touches;
					if( touches && touches.length ) {
		...
		var unbindTouchSwipe = function() {
			...
			$(window).unbind( "touchmove.vxSlider" );
		};
		...
		var bindMouseWheelEvent = function() {
			$(window).bind('wheel.vxSlider mousewheel.vxSlider DOMMouseScroll.vxSlider MozMousePixelScroll.vxSlider', mouseWheelHandler );
		};
		...
		var bindKeyArrows = function() {
			...
			.bind( 'focus.vxSlider', function(){ allow = false; })
			.bind( 'blur.vxSlider', function(){ allow = true; });
		...
		var app = {
			...
			addPagination : function() {
				pagination = $('<ul id="vx-slider-pagination"></ul>');
				$( options.selector, body ).each( function(i) {
				...
				if( $('#vx-slider-pagination').length !== 0 ) {
					$('#vx-slider-pagination').remove();
				}
				...
				$('li', pagination).bind( 'click.vxSlider', function(e){
